...
- **RabbitMQ под Celery быстрее Redis в 1,5–1,8 раза** на коротких задачах и на постановке. Результат контринтуитивный — на следующем слайде разбор, почему так вышло.
...
## Проверка результата: почему Redis «проиграл»
> Интуиция говорила обратное — разобрал замер по частям

| Замер из контейнера воркера | Результат |
|---|---|
| Redis: `PING` / `LPUSH` по одной | 10 300 / 9 100 оп/с (≈110 мкс round-trip) |
| Redis: `LPUSH` пачками по 100 (pipeline) | 106 700 оп/с |
| Celery: публикация задачи → Redis | 1 169 оп/с (855 мкс) |
| Celery: публикация задачи → RabbitMQ | 1 901 оп/с (526 мкс) |
| Celery: → RabbitMQ + `confirm_publish=True` | **218 оп/с** (4 584 мкс) |

- **Redis работал на ~10 % своего потолка**: упирались не в него, а в число синхронных round-trip'ов — транспорт kombu эмулирует ack четырьмя командами на задачу.
- **Главная цена — сам Celery** (380–570 мкс на сборку сообщения), а не брокер: RQ на том же Redis ставит 2 300 задач/с.
- **Асимметрия замера**: в `celery-redis` один однопоточный Redis обслуживал брокер + события Flower + result backend — 26 команд на задачу против 13 у конфигурации с RabbitMQ.
- **Преимущество AMQP куплено гарантией**: publish по умолчанию не подтверждается; включаем confirms — RabbitMQ в 8 раз медленнее Redis.

!> Правильная формулировка: медленнее не Redis, а Redis-**транспорт** kombu в этой конфигурации. Вывод для выбора не меняется — RabbitMQ берут за гарантии, а не за скорость.

::: notes
Этот слайд — про то, что с собственным результатом нужно спорить. Диагностика воспроизводима: bench/diagnose.py, раздел «Аудит» в docs/experiment.md.
Данные матрицы при этом корректны: во всех 135 прогонах число живых воркеров совпало с ожидаемым, разброс между повторами — 2 % (максимум 17 %), то есть меньше сравниваемых разниц.
Что стоило сделать иначе: отдельный Redis под результаты и метрики, прогон без событий Flower, сравнение при одинаковых гарантиях публикации.
